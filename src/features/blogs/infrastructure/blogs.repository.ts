...
export class BlogsRepository {
  ...
  public async create(blog: CreateBlogInputModel): Promise<BlogOutputModel> {
    const createdBlog = await this.blogModel.create(blog)
    return BlogOutputMapper(createdBlog)
  }

  public async updateBlog(blogId: string, updatedBlog: UpdateBlogInputModel): Promise<boolean> {
